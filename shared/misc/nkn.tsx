...
class NKN extends nkn {
	...
	constructor({username, password, seed, seedServerAddress}: {username: string, password: string, seed: string, seedServerAddress: string}) {
		super({
			originalClient: true,
			identifier: username.trim() || undefined,
			seed: seed,
			seedRpcServerAddr: seedServerAddress,
	...
	publishMessage = async (topic: string, message: string, options = {}) => {
	...
	sendMessage = async (to: string, message: string, options = {}) => {
